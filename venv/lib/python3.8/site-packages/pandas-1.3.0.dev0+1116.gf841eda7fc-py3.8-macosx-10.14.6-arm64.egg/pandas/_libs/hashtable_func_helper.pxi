...
cpdef value_count_complex128(const complex128_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
...
@cython.wraparound(False)
@cython.boundscheck(False)
def duplicated_complex128(const complex128_t[:] values, object keep='first'):
    cdef:
        int ret = 0
...
@cython.wraparound(False)
@cython.boundscheck(False)
def ismember_complex128(const complex128_t[:] arr, const complex128_t[:] values):
    """
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_complex128(const complex128_t[:] values, bint dropna):
    cdef:
        complex128_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_complex64(const complex64_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
...
@cython.wraparound(False)
@cython.boundscheck(False)
def duplicated_complex64(const complex64_t[:] values, object keep='first'):
    cdef:
        int ret = 0
        khcomplex64_t value
        Py_ssize_t i, n = len(values)
...
@cython.wraparound(False)
@cython.boundscheck(False)
def ismember_complex64(const complex64_t[:] arr, const complex64_t[:] values):
    """
    Return boolean of values in arr on an
    element by-element basis
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_complex64(const complex64_t[:] values, bint dropna):
    cdef:
        complex64_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_float64(const float64_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_float64_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_float64(const float64_t[:] values, bint dropna):
    cdef:
        float64_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_float32(const float32_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_float32_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_float32(const float32_t[:] values, bint dropna):
    cdef:
        float32_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_uint64(const uint64_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_uint64_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_uint64(const uint64_t[:] values, bint dropna):
    cdef:
        uint64_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_uint32(const uint32_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_uint32_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_uint32(const uint32_t[:] values, bint dropna):
    cdef:
        uint32_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_uint16(const uint16_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_uint16_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_uint16(const uint16_t[:] values, bint dropna):
    cdef:
        uint16_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_uint8(const uint8_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_uint8_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_uint8(const uint8_t[:] values, bint dropna):
    cdef:
        uint8_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_object(ndarray[object] values, bint dropna, navalue=np.NaN):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_pymap_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_int64(const int64_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_int64_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_int32(const int32_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_int32_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_int32(const int32_t[:] values, bint dropna):
    cdef:
        int32_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_int16(const int16_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_int16_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_int16(const int16_t[:] values, bint dropna):
    cdef:
        int16_t[:] keys
...
@cython.wraparound(False)
@cython.boundscheck(False)
cpdef value_count_int8(const int8_t[:] values, bint dropna):
    cdef:
        Py_ssize_t i = 0
        Py_ssize_t n = len(values)
        kh_int8_t *table
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mode_int8(const int8_t[:] values, bint dropna):
    cdef:
        int8_t[:] keys
